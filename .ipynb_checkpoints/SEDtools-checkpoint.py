...
class SEDProperties:
    ...
    def plot_SED(self, ax_SED, identifier, \
                 ...
                 SED_observed_color = 'tab:red', SED_model_color = 'k', SED_model_lw = 3.0, obs_ms = 5, \
                 SED_uplim_color = 'k', SED_uplim_alpha = 0.8, \
                 xmin = 1E-1, xmax = 3E5, ymin = -8, ymax = 2):
        
        from matplotlib.ticker import (MultipleLocator, FormatStrFormatter, AutoMinorLocator)

        ax_SED.plot(self.lambda_model.to('micrometer'), np.log10(self.SED_model.value), color = SED_model_color, linewidth = SED_model_lw)
        if plot_torus:
        ...
        if plot_uplims:
            ...
            ax_SED.errorbar(x[self.GOOD_UpLim], np.log10(self.SED_observed_errors.value)[self.GOOD_UpLim], \
                            yerr=0.2, color=SED_uplim_color, alpha = SED_uplim_alpha, fmt='none', ms=15, zorder=3, capsize=3, uplims=True)
        # Scales and whatever
...
def band_disambiguation(band):
    ...
    if band in ['AzTEC','AZTEC1p1', 'MAMBO', 'MAMBO250', 'MAMBO250.', '1160']: return 'AzTEC+MAMBO'
    if band in ['VLA10cm', 'VLA3GHz', '3GHz']: return '10cm'
    if band in ['MIGHTEE', 'VLA20cm', 'VLA_L', 'VLA1.4GHz', 'VLA14GHz', '1_4GHz']: return '20cm'
    return band
...
Wvlghts_dictionary = {'GALEX_FUV': 0.1528, 'GALEXFUV': 0.1528, 'GALEX_NUV': 0.2271, 'GALEXNUV': 0.2271, 'u': 0.3551, 'KPNO_U': 0.3593, 'LBC_U': 0.3633, \
                    ...
                    'HSC_r': 0.622, 'HSC_i': 0.773, 'HSC_z': 0.891, 'HSC_Y': 0.976, \
                    'Y': 1.022, 'J': 1.253, 'H': 1.643, 'Ks': 2.147, \
